...
# just the numbers
def plot_corrs_per_era_new(
    df=None, pred_name="prediction", target_name="target", legend_title=None
):
...
def print_metrics_new(
    ...
    if long_metrics:
        ...
        if val_df is not None:
            ...
            feature_neutral_mean = utils.get_feature_neutral_mean(
...
def run_feature_neutralization_new(
    df=None,
    ...
    if no_fn:
        ...
        metrics = print_metrics_new(
            tour_df=df, pred_name=pred_name, target_name=target_name, long_metrics=False
        )
        ...
        if neut_type == "short":
            df[pred_name + "_neutralized"] = fn.neutralize_short(
                df, prediction_name=pred_name, proportion=proportion
            )
        elif neut_type == "perera":
            df[pred_name + "_neutralized"] = fn.neutralize(
                df=df,
                columns=[pred_name],
        ...
        validation_data = df[df["data_type"] == "validation"]
        val_corrs = utils.corr_score(
            validation_data, pred_name + "_neutralized", target_name
        )
        sharpe = utils.sharpe_score(val_corrs)
        print(
            "Validation correlations : {}\n"
...
def plot_feature_neutralization_new(
    tour_df,
    neut_percent,
    full=False,
    feature_names=None,
    ...
    if plot_feature_exposures:
        ...
        ax.legend(
            ...
            "Mean feature exposure : {}".format(
                utils.max_feature_exposure(validation_data, pred_name),
    ...
    if neutralize_total:
        ...
        validation_data[pred_name + "_neutralized"] = fn.neutralize_short(
            validation_data, prediction_name=pred_name, proportion=neut_percent
        )

        feat_exps, feats = utils.feature_exposures(
            validation_data, pred_name + "_neutralized"
        )
        ...
        val_corrs_neut = utils.corr_score(
            validation_data, pred_name + "_neutralized", target_name
        )
    ...
    if neutralize_per_era:
        ...
        feat_exps, feats = utils.feature_exposures(
            validation_data, PRED_NAME_NEUT_PER_ERA
        )
    ...
    if neutralize_total:
        ...
        if legend_title is not None:
            plot_corrs_per_era_new(
                validation_data, pred_name + "_neutralized", legend_title[1]
            )
        else:
    ...
    if neutralize_per_era:
        ...
        if legend_title is not None:
            plot_corrs_per_era_new(
                validation_data, PRED_NAME_NEUT_PER_ERA, legend_title[2]
            )
        else:
