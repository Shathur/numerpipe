...
class EraBooster:
    ...
    def train_once(self, iter_count, train_data, test_data):
        print(
            f"Adding {self.added_trees} trees and training on worst eras ... Iter: {iter_count}"
        )
    ...
    def return_era_scores(self, train_df, test_df):
        ...
        for era in era_scores_train.index:
            ...
            era_scores_train[era] = spearman(
                era_df[self.prediction_col], era_df[self.target_col]
            )  # .correlation
    ...
    def era_diff(self, test_df, era_scores_test):
        ...
        if self.iteration > 0:
            for era in era_scores_test.index:
                era_diff_test[era] = (
                    era_scores_test[era] - self.era_scores_test_prior[era]
                )
    ...
    def worst_scores(self, era_scores_train):
        ...
        worst_eras = era_scores_train[
            era_scores_train <= era_scores_train.quantile(self.quantile)
        ].index.tolist()
