...
def get_model_lst(num_models=1, prefix=None, folder_name=None, verbose=True):
    ...
    model_lst = [
        os.path.join(folder_name, x) for x in os.listdir(os.path.join(folder_name))
    ]
...
def run_model(
    ...
    # check if our save folder exists
    if save_to_drive:
        assert (
            save_folder is not None
        ), "save_folder must be provided when save_to_drive is True"
        assert os.path.exists(save_folder), "save_folder does not exist"
    ...
    if save_to_drive:
        if legacy_save:
            ...
            if model_type == "lgb":
                ...
                model.save_model(
                    Path(Path(save_folder) / "model_{}.json".format(cv_count))
                )
    # else:
    #     if legacy_save:
