        ...
        if n_splits > 1:
            ...
            yield (
                indices[
                    groups.isin(
                        group_list[
                            : int(self.single_split_ratio * len(group_list))
                            - embg_grp_num
                        ]
                    )
                ],
...
def cross_validate_train(
    ...
    for cv_count, idx_cv in enumerate(cv_split_data):
        ...
        if calculate_metrics:
            # predict
            predictions_train = predict_in_era_batch(
                model,
                train_data[feature_names],
                era_idx=era_idx_train,
                rank_per_era=True,
            )
            predictions_val = predict_in_era_batch(
                model, val_data[feature_names], era_idx=era_idx_val, rank_per_era=True
            )
            ...
            if (tour_df is not None) and (not tour_df.empty):
                predictions_tour = predict_in_era_batch(
                    model,
                    tour_df[feature_names],
                    era_idx=era_idx_tour,
                    rank_per_era=True,
                )
            ...
            if plot_metrics:
                plot_corrs_per_era_new(
                    df=val_data, pred_name=PREDICTION_NAME, target_name=target_name
                )
                print(120 * "*")
                if (tour_df is not None) and (not tour_df.empty):
                    plot_corrs_per_era_new(
                        df=tour_df, pred_name=PREDICTION_NAME, target_name=target_name
                    )
            ...
            val_correlations, tour_correlations, val_sharpe_cv, tour_sharpe_cv = [
                0,
                0,
                0,
                0,
