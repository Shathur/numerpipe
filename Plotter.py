...
def compare(models_lst, round, metric_to_plot):
    ...
    for model_id in models_lst:
        ...
        # add corr+mmc
        model_daily_df["corr+mmc"] = (
            model_daily_df["correlation"] + model_daily_df["mmc"]
        )
        model_round = model_daily_df[
            model_daily_df["roundNumber"] == round
        ].reset_index(drop=True)
        model_round["id"] = model_id
        models_df_lst.append(model_round)

    models_melted = pd.melt(
        pd.concat(models_df_lst).reset_index(drop=True),
        id_vars=["id", "date"],
    ...
    plt.figure()
    sns.lineplot(
        x="date", y="value", hue="id", data=models_melted, sort=False
    )  # hue='variable
...
def compare(models_lst, round, metric_to_plot):
    ...
    sns.lineplot(
        ax=axes[0], x="date", y="value", hue="id", data=models_melted, sort=False
    )  # hue='variable
    plt.xticks(rotation=45)
    plt.xlim(0)
    plt.show()
...
class Plotter:
    ...
    def model_daily_scores(self, model_id):
        ...
        model_daily_df = pd.DataFrame(model_daily)
        # add corr+mmc
        model_daily_df["corr+halfmmc"] = (
            model_daily_df["correlation"] + 0.5 * model_daily_df["mmc"]
        )
        model_daily_df["corr+mmc"] = (
            model_daily_df["correlation"] + model_daily_df["mmc"]
        )
        model_daily_df["corr+2mmc"] = (
            model_daily_df["correlation"] + 2 * model_daily_df["mmc"]
        )
        model_round = model_daily_df[
            model_daily_df["roundNumber"].isin(self.round_id)
        ].reset_index(drop=True)
        model_round["id"] = model_id
        return model_round
    ...
    def plot_single_round(self):
        models_melted = pd.melt(
            pd.concat(self.get_models_df_lst()).reset_index(drop=True),
            id_vars=["id", "date"],
    ...
    def plot_subplot(self):
        ...
        for rnd in self.round_id:
            ...
            models_melted = pd.melt(
                model_rnd, id_vars=["id", "date"], value_vars=[self.metric_to_plot]
            )
            models_melted["date"] = models_melted["date"].apply(self.fix_format)
            ...
            sns.lineplot(
                ax=ax, x="date", y="value", hue="id", data=models_melted, sort=False
            )  # hue='variable
            plt.show()
