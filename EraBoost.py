...
def era_boost(train_df, tour_df, X, y, _trees_per_step, _iterations, _era_quantile):
    ...
    for i in range(_iterations):
        if i == 0:
            ...
            _model.fit(
                worst_eras_df[X.columns], worst_eras_df[TARGET_NAME], xgb_model=booster
            )
        ...
        for era in era_scores_train.index:
            ...
            era_scores_train[era] = spearmanr(
                era_df[PREDICTION_NAME], era_df[TARGET_NAME]
            ).correlation

        for era in era_scores_tour.index:
            era_df = tour_df[tour_df["era"] == era]
            era_scores_tour[era] = spearmanr(
                era_df[PREDICTION_NAME], era_df[TARGET_NAME]
            ).correlation
        ...
        if i > 0:
            for era in era_scores_tour.index:
                ...
                dic = {
                    "iteration": "it" + str(i),
                    "era": "_" + era[3:],
                    "correlation_changes": era_diff_tour[era],
                }
        ...
        worst_eras = era_scores_train[
            era_scores_train <= era_scores_train.quantile(_era_quantile)
        ].index
...
def era_boost_val_criterion(
    model, train_df, tour_df, X, y, _trees_per_step, _era_quantile, stop_criterion=20
):
    ...
    while loop_condition:
        if iter_count == 0:
            ...
            print(
                "Adding trees and training on worst eras ... Iter : {}".format(
                    iter_count
                )
            )
            _model.n_estimators += _trees_per_step
        ...
        preds_train = predict_in_era_batch(
            _model, X, era_idx=era_idx_train, rank_per_era=True
        )
        train_df[PREDICTION_NAME] = preds_train

        preds_tour = predict_in_era_batch(
            _model, tour_df[X.columns], era_idx=era_idx_val, rank_per_era=True
        )
        tour_df[PREDICTION_NAME] = preds_tour
    ...
    sns.lineplot(
        x="iteration", y="value", hue="variable", data=keep_sharpe_df_for_lineplot
    )
